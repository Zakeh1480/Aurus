...
MVP funcionalmente completo, construído em 19 iterações disciplinadas ("um prompt = um PR"). HEAD atual é pós-Prompt 19 (pareamento/aceite do matchmaking migrado para Redis).
...
- Redis adapter do Socket.IO instalado (Prompt 17) — entrega de eventos WS por usuário já é cross-instance-safe. O _pareamento/aceite_ do matchmaking (lacuna dos Prompts 17/18) também é cross-instance-safe agora, via `PendingMatchService`/Redis (Prompt 19). Mas os schedulers em memória de `scoring` (`ScoreTickSchedulerService`), `anti-cheat` (`ChallengeSchedulerService`) e `livekit` (`MatchDurationSchedulerService`) seguem com a mesma limitação (`Map`/`Set` local) — API ainda não é 100% multi-réplica.
