...
MVP funcionalmente completo, construído em 18 iterações disciplinadas ("um prompt = um PR"). HEAD atual é pós-Prompt 17 (Redis adapter para Socket.IO).
...
- Prompt 18 — rotação de `AI_SERVICE_SHARED_SECRET` com chave anterior: `services/ai` aceita `AI_SERVICE_SHARED_SECRET_PREVIOUS` opcional como fallback, permitindo rollout gradual (deploy de `services/ai` antes de `apps/api`) sem deploy simultâneo dos dois lados ← estado atual
...
- Redis adapter do Socket.IO instalado (Prompt 17) — entrega de eventos WS por usuário já é cross-instance-safe. Mas `MatchmakingService.pendingMatches`/`pendingMatchByUser` (pareamento/aceite-com-timeout) ainda vivem em `Map` local → API segue restrita a **1 réplica** até esse estado também migrar para Redis.
- Rotação de `AI_SERVICE_SHARED_SECRET` já suporta "chave anterior" (`AI_SERVICE_SHARED_SECRET_PREVIOUS`, Prompt 18) — mas segue sendo um único valor anterior, não uma lista; rotações encadeadas (trocar de novo antes de remover a anterior) não são suportadas.
