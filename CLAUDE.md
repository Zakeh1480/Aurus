...
MVP funcionalmente completo, construído em 20 iterações disciplinadas ("um prompt = um PR"). HEAD atual é pós-Prompt 20 (encerramento por duração de partida migrado para Redis).
...
- Redis adapter do Socket.IO instalado (Prompt 17) — entrega de eventos WS por usuário já é cross-instance-safe. O _pareamento/aceite_ do matchmaking (lacuna dos Prompts 17/18) também é cross-instance-safe agora, via `PendingMatchService`/Redis (Prompt 19), assim como o encerramento forçado por duração de partida, via o ZSET `lk:match-duration:expiry`/Redis (Prompt 20). Mas os schedulers em memória de `scoring` (`ScoreTickSchedulerService`) e `anti-cheat` (`ChallengeSchedulerService`) seguem com a mesma limitação (`Map`/`Set` local) — API ainda não é 100% multi-réplica.
