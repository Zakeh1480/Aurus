...
MVP funcionalmente completo, construído em 22 iterações disciplinadas ("um prompt = um PR"), seguidas por uma rodada de prompts fechando lacunas que não dependem de decisão/credencial humana. HEAD atual é pós-Prompt 25 (gate de severidade bloqueando o pipeline por achado de segurança).
...
- CI é 100% GitLab (`.gitlab-ci.yml`): stages `test` (lint, typecheck, test, ai-service, sast, secret_detection, dependency_scanning), `build` (build, docker-build, container_scanning_api, container_scanning_ai) e `security-gate` (gate de severidade, Prompt 25) — **sem job de deploy**; deploy é via integração nativa Railway/Vercel configurada manualmente.
