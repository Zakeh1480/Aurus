...
MVP funcionalmente completo, construído em 22 iterações disciplinadas ("um prompt = um PR"), seguidas por uma rodada de prompts fechando lacunas que não dependem de decisão/credencial humana. HEAD atual é pós-Prompt 24 (imagens de `apps/api`/`services/ai` publicadas no GitLab Container Registry do projeto + Container Scanning no CI).
...
- CI é 100% GitLab (`.gitlab-ci.yml`): stages `test` (lint, typecheck, test, ai-service, sast) e `build` (build, docker-build, container_scanning_api, container_scanning_ai) — **sem job de deploy**; deploy é via integração nativa Railway/Vercel configurada manualmente.
...
- SAST/Secret Detection/Dependency Scanning/Container Scanning rodam mas não bloqueiam o pipeline por achado (comportamento padrão do GitLab, só informam a MR) — um script customizado que parseia os relatórios e falha o job acima de um limiar de severidade é uma alternativa gratuita ao GitLab Ultimate.
