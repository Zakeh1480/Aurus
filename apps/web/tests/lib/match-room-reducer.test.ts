...
import {
  ...
  type MatchRoomState,
} from '../../src/lib/match-room-reducer.js';
...
function matchResult(winnerId: string | null): MatchResultPayload {
  const score = {
    overall: 0.7,
    breakdown: { posture: 0.7, eyeContact: 0.7, expression: 0.7, presence: 0.7, movement: 0.7 },
...
describe('reduceMatchRoomState', () => {
  ...
  it('connecting -> error em CONNECTION_FAILED', () => {
    const next = reduceMatchRoomState(initialMatchRoomState, {
  ...
  it('mantém o último score válido se um SCORE_TICK não contiver o próprio userId', () => {
    const withScores: MatchRoomState = {
      status: 'live',
      scores: { self: 0.4, opponent: 0.9, opponentId: OPPONENT },
    ...
    const next = reduceMatchRoomState(withScores, {
      type: 'SCORE_TICK',
      payload: badTick,
      selfUserId: SELF,
    });
  ...
  it('VERIFY_CHALLENGE_ANSWERED limpa o challenge pendente só se o id bater', () => {
    ...
    const wrongId = reduceMatchRoomState(withChallenge, {
      type: 'VERIFY_CHALLENGE_ANSWERED',
      challengeId: 'chal-2',
    });
    expect(wrongId).toBe(withChallenge);

    const rightId = reduceMatchRoomState(withChallenge, {
      type: 'VERIFY_CHALLENGE_ANSWERED',
      challengeId: 'chal-1',
    });
  ...
  it.each(['completed', 'forfeited'] as const)(
    'MATCH_END com reason %s vai para ended-awaiting-result, carregando o reason',
    (reason) => {
      const next = reduceMatchRoomState(liveState, {
        type: 'MATCH_END',
  ...
  it.each(['disconnected', 'cancelled'] as const)(
    'MATCH_END com reason %s vai para ended-no-result',
    (reason) => {
      const next = reduceMatchRoomState(liveState, {
        type: 'MATCH_END',
        payload: { matchId: MATCH_ID, endedAt: '2026-01-01T00:02:00.000Z', reason },
  ...
  it('MATCH_RESULT direto em live (sem match:end prévio) assume reason completed', () => {
    const result = matchResult(SELF);
    const next = reduceMatchRoomState(liveState, { type: 'MATCH_RESULT', payload: result });
    expect(next).toEqual({ status: 'result', result, reason: 'completed' });
  });

  it.each(['completed', 'forfeited'] as const)(
    'ended-awaiting-result -> result em MATCH_RESULT, repassando o reason (%s)',
    (reason) => {
      const awaiting: MatchRoomState = {
        status: 'ended-awaiting-result',
        endedAt: '2026-01-01T00:02:00.000Z',
        reason,
      };
      ...
      expect(next).toEqual({ status: 'result', result, reason });
    },
  );
  ...
  it('estados terminais (ended-no-result, result, error) ignoram qualquer ação', () => {
    const terminals: MatchRoomState[] = [
      ...
      { status: 'result', result: matchResult(null), reason: 'completed' },
