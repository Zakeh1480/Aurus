...
function useAttachedTrack(track: AttachableTrack | null, videoRef: React.RefObject<HTMLVideoElement | null>): void {
  React.useEffect(() => {
...
function Tile({ label, videoRef, muted, placeholderMessage }: TileProps) {
  return (
    <div className="relative aspect-video overflow-hidden rounded-lg border border-border bg-muted">
      ...
      <span className="absolute bottom-2 left-2 rounded bg-background/70 px-2 py-0.5 text-xs font-medium">{label}</span>
    </div>
  );
}
...
/**
 * `localVideoRef` vem de fora (MatchRoom) porque o mesmo elemento de vídeo é
 * lido pelo MediaPipe (useAuraFeatures) e pela captura de snapshot do
 * challenge — precisa ser o vídeo local de verdade, não uma cópia.
 */
