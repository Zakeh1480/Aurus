...
/**
 * Ao receber MATCH_END em "matched", o próximo estado depende do que o
 * servidor já fez silenciosamente (ver cancelPendingMatch em
 * apps/api/src/matchmaking/matchmaking.service.ts): quem pediu para sair é
 * sempre removido da fila; senão, quem já tinha aceitado é reinserido.
 */
function resolveMatchEndState(state: Extract<MatchmakingState, { status: "matched" }>): MatchmakingState {
...
export function reduceMatchmakingState(state: MatchmakingState, action: MatchmakingAction): MatchmakingState {
...
/** Extrai a mensagem do payload cru do evento "exception" (fora de WsEventSchemas). */
