...
describe("JwtStrategy", () => {
  ...
  beforeEach(async () => {
    // JwtStrategy chama getJwtSecret() no construtor (via super()) — precisa
    // estar setado antes do módulo compilar, já que não há .env carregado em testes.
  ...
  it("lança quando o usuário não existe mais", async () => {
    ...
    await expect(strategy.validate({ sub: "user-fantasma" })).rejects.toBeInstanceOf(UnauthorizedException);
  });
