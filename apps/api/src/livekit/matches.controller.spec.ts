...
describe("MatchesController", () => {
  ...
  let prisma: { match: { findUnique: ReturnType<typeof vi.fn> }; matchResult: { findUnique: ReturnType<typeof vi.fn> } };
  let livekit: {
  ...
  describe("getScoreExplanation", () => {
    ...
    it("lança 403 para um terceiro sem role de moderador", async () => {
      ...
      await expect(controller.getScoreExplanation(MATCH_ID, buildUser({ id: USER_C }))).rejects.toBeInstanceOf(
        ForbiddenException,
      );
    });
    ...
    it("lança 404 quando o resultado da partida ainda não existe", async () => {
      ...
      await expect(controller.getScoreExplanation(MATCH_ID, buildUser({ id: USER_A }))).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
  });
