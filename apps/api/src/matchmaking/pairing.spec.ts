...
describe("selectBestCandidate", () => {
  ...
  it("desempata a favor de quem espera há mais tempo", () => {
    ...
    // ambos a 10 de distância — vence quem esperou mais
  ...
  it("inclui um candidato distante quando a janela LARGA do self alcança (espera longa do self)", () => {
    const longWaitingSelf: QueueMember = { userId: "self", rating: 1000, waitMs: 60_000 }; // janela self = 350
    const candidates: QueueMember[] = [{ userId: "fresh", rating: 1300, waitMs: 0 }]; // janela candidato = 50, diff=300
  ...
  it("inclui um self recém-chegado quando a janela LARGA do candidato que espera há muito alcança", () => {
    const candidates: QueueMember[] = [{ userId: "veteran", rating: 1300, waitMs: 60_000 }]; // janela candidato = 350
    expect(selectBestCandidate(self, candidates, CONFIG)?.userId).toBe("veteran");
  });
});
