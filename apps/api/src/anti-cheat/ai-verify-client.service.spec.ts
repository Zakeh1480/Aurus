...
describe("AiVerifyClientService", () => {
  ...
  beforeEach(async () => {
    const moduleRef = await Test.createTestingModule({ providers: [AiVerifyClientService] }).compile();
  ...
  it("retorna a VerifyResponse quando a chamada é bem-sucedida", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({ ok: true, status: 200, json: () => Promise.resolve(VALID_RESPONSE_BODY) }),
    );
