...
export function getAntiCheatConfig(): AntiCheatConfig {
  return {
    ...
    temporalViolationSaturation: Number(process.env["ANTI_CHEAT_TEMPORAL_VIOLATION_SATURATION"] ?? 5),
    statsTtlSeconds: Number(process.env["ANTI_CHEAT_STATS_TTL_SECONDS"] ?? 3600),
    trustHighMin: Number(process.env["ANTI_CHEAT_TRUST_HIGH_MIN"] ?? ANTI_CHEAT_TRUST_THRESHOLDS.highMin),
    trustLowMax: Number(process.env["ANTI_CHEAT_TRUST_LOW_MAX"] ?? ANTI_CHEAT_TRUST_THRESHOLDS.lowMax),
    trustWeights: {
