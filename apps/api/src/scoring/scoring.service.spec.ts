...
function buildTrustAssessment(userId: string, decision: "valid" | "flagged" | "discarded") {
  return {
    ...
    trustLevel: decision === "valid" ? ("high" as const) : decision === "flagged" ? ("medium" as const) : ("low" as const),
    decision,
    discrepancyAvg: 0.05,
    livenessFlagCounts: {
...
describe("ScoringService.finalizeMatch", () => {
  ...
  let matchmakingService: { emitToUser: ReturnType<typeof vi.fn>; endActiveMatch: ReturnType<typeof vi.fn> };
  ...
  beforeEach(async () => {
    tx = {
      profile: {
        findUniqueOrThrow: vi
          .fn()
          .mockResolvedValueOnce({ userId: PLAYER_1, nickname: "Player One", auraScoreAvg: null, matchesPlayed: 0, wins: 0, losses: 0 })
          .mockResolvedValueOnce({ userId: PLAYER_2, nickname: "Player Two", auraScoreAvg: null, matchesPlayed: 0, wins: 0, losses: 0 }),
        update: vi
          .fn()
      ...
      matchResult: {
        create: vi.fn().mockResolvedValue({ id: RESULT_ID, createdAt: new Date("2026-01-01T00:05:01.000Z") }),
      },
    ...
    prisma = {
      match: {
        findUnique: vi
          .fn()
          .mockResolvedValue({ id: MATCH_ID, status: "active", player1Id: PLAYER_1, player2Id: PLAYER_2 }),
      },
      matchParticipant: {
        findMany: vi
          .fn()
    ...
    aiScoreClient = {
      aggregate: vi.fn().mockResolvedValueOnce(buildScore(0.8)).mockResolvedValueOnce(buildScore(0.4)),
    };
    scoreTickScheduler = { cancel: vi.fn() };
    matchmakingService = { emitToUser: vi.fn(), endActiveMatch: vi.fn().mockResolvedValue(undefined) };
  ...
  it("partida simulada ponta-a-ponta: agrega score, persiste resultado, atualiza rating e ranking, emite end+result", async () => {
    ...
    expect(rankingService.recordMatchResult).toHaveBeenCalledWith([
      { userId: PLAYER_1, displayName: "Player One", rating: 1016, auraScoreAvg: 0.8, matchesPlayed: 1 },
      { userId: PLAYER_2, displayName: "Player Two", rating: 984, auraScoreAvg: 0.4, matchesPlayed: 1 },
    ]);
  ...
  it("idempotente: não faz nada se a partida já não está active", async () => {
    prisma.match.findUnique.mockResolvedValue({ id: MATCH_ID, status: "completed", player1Id: PLAYER_1, player2Id: PLAYER_2 });
