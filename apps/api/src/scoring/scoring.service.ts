import { type AuraScore, MatchResultSchema, type WsEventPayload } from "@aurafarming/shared";
...
import { computeMatchOutcome, type MatchOutcome } from "./match-outcome";
...
export class ScoringService {
  ...
  async finalizeMatch(matchId: string): Promise<void> {
    ...
    if (decision.overallDecision === "discarded") {
      this.logger.log(`Partida ${matchId} descartada por trust score baixo — rating/ranking não são tocados.`);
    ...
    const winnerId =
      outcome.winnerSide === "player1" ? match.player1Id : outcome.winnerSide === "player2" ? match.player2Id : null;
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
        this.logger.warn(`Partida ${matchId} já foi finalizada por outra chamada concorrente — no-op.`);
        return;
      }
      throw error;
    }
    ...
    const endPayload: WsEventPayload<"match:end"> = { matchId, endedAt: endedAtIso, reason: "completed" };
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const profile1 = await tx.profile.update({
        ...
        data: {
          ...
          auraScoreAvg: runningAverage(profile1Before.auraScoreAvg, profile1Before.matchesPlayed, score1.overall),
      ...
      const profile2 = await tx.profile.update({
        ...
        data: {
          ...
          auraScoreAvg: runningAverage(profile2Before.auraScoreAvg, profile2Before.matchesPlayed, score2.overall),
