...
describe("MatchScoringGateway", () => {
  ...
  it("na primeira mensagem de uma partida active, agenda o tick com os dois player IDs", async () => {
    await gateway.onFeatures(fakeSocket(USER_ID), featuresPayload());

    expect(scoreTickScheduler.ensureScheduledForMatch).toHaveBeenCalledWith(MATCH_ID, "player-1", "player-2");
  });

  it("não consulta o Prisma nem reagenda a partir da segunda mensagem da mesma partida", async () => {
    await gateway.onFeatures(fakeSocket(USER_ID), featuresPayload());
    await gateway.onFeatures(fakeSocket(USER_ID), featuresPayload({ features: { ...featuresPayload().features, sequence: 1 } }));
