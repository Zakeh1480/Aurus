...
function buildScore(overrides: Partial<AuraScore["breakdown"]> = {}): AuraScore {
  const breakdown = { posture: 0.8, eyeContact: 0.7, expression: 0.6, presence: 0.9, movement: 0.5, ...overrides };
  // overall calculado a partir do breakdown (nunca hardcoded) — mantém a fixture
  // consistente mesmo quando um teste sobrescreve valores individuais.
  const overall = AURA_METRIC_KEYS.reduce((total, key) => total + breakdown[key] * AURA_SCORE_WEIGHTS[key], 0);
...
describe("explainMetrics", () => {
  ...
  it("contribution = raw × weight para cada métrica", () => {
    const score = buildScore({ posture: 0.4, eyeContact: 1, expression: 0, presence: 0.25, movement: 0.6 });
    const metrics = explainMetrics(score);
    for (const metric of metrics) {
