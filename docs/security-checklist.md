...
- [x] Cabeçalhos de segurança em `apps/web` (`next.config.ts`): `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy` (câmera liberada só para o próprio origin), `Strict-Transport-Security`, e desde o Prompt 28 também `Content-Security-Policy` (allowlist explícita: próprio origin, API/WS configurados, `*.livekit.cloud`, CDN do MediaPipe WASM/modelo — `'wasm-unsafe-eval'` liberado só porque a extração de landmarks roda via WASM no cliente, regra de ouro 5). Desde o Prompt 31, `script-src` usa nonce por requisição (`apps/web/src/middleware.ts` + `apps/web/src/lib/csp.ts`) em vez de depender só do allowlist estático — o CSP original do Prompt 28 não liberava `'unsafe-inline'` nem nonce/hash, então bloqueava os próprios scripts inline que o Next.js injeta para hidratar Server Components, quebrando a hidratação em produção sem que nenhum teste local pegasse (gap de E2E/browser já documentado).
