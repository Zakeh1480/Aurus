...
- [ ] Rotação do segredo compartilhado: gerar um novo valor, atualizar `AI_SERVICE_SHARED_SECRET` nos dois serviços **no mesmo deploy** (um lado desatualizado quebra scoring/verify inteiro — não há verificação de "chave anterior" para rollout gradual).
